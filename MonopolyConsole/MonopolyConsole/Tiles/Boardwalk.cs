            ...
            if(!IsOwned) // if property is not owned..
            {
                ...
                if (input.Equals("yes"))
                {
                    if (player.GetBalance() >= PropertyCost)
                    {
                        ...
                        Console.WriteLine($"{player.GetPlayerName()} does not have sufficient funds.");
                }
                else
                    Console.WriteLine($"{player.GetPlayerName()} did not purchase {Name}"); // player did not want to buy
            }
